...
GameObject instd = Instantiate(testPrefab, peak, Quaternion.Euler(0,0,0));
        for (int i = 0, z = 0; z <= mapZ; z++)
        {
            for (int x = 0; x <= mapX; x++)
            {
                float distZ = Vector3.Distance(peak, new Vector3(peak.x, 0, z * squareSize));
                float distX = Vector3.Distance(peak, new Vector3((x * squareSize), 0, peak.z - Mathf.Pow(distZ/30f, 2f)));
                // float dist = Vector3.Distance(peak, new Vector3(x * squareSize, 0, z * squareSize));
                float y = Mathf.PerlinNoise(x * .12f, z * .12f) * noiseStrength * 9  - Mathf.Sqrt(distX * 6 * centralHillStrength) + 290f;
        ...
        for (int i = 0; i < vertices.Length; i++)
            colors[i] = Color.Lerp(
                ...
                Color.Lerp(
                    Color.green, 
                    CombineColors(CombineColors(Color.black, Color.green), Color.gray),
                    (vertices[i].y - 40) / 3),
                vertices[i].y + 15);
